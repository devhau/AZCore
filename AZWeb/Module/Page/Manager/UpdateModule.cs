        ...
        where TService:EntityService<TService,TModel>
    {
        ...
        public virtual void DataFormToData(TModel DataForm, Func<PropertyInfo, bool> funProper = null)
        {
            ...
            foreach (var item in ModelType.GetProperties()) {
                if ((this.HttpContext.Request.Form.ContainsKey(item.Name)||(this.HttpContext.Request.Form.Files.GetFiles(item.Name).Count>0&&item.GetAttribute<FieldUploadFileAttribute>()!=null)) && (funProper==null||funProper(item))) 
                {
